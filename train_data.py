...
def test_sim(support, data, model, device):
    # 预分配numpy数组
    total_pairs = support.shape[0] * data.shape[0]
    pairs = np.zeros((total_pairs, 2, 80, 6))
    
    # 直接使用numpy数组
    support_np = support.cpu().numpy() if torch.is_tensor(support) else support
    data_np = data.cpu().numpy() if torch.is_tensor(data) else data
    
    # 填充pairs数组
    idx = 0
    for d in range(support_np.shape[0]):
        for i in range(data_np.shape[0]):
            pairs[idx, 0] = support_np[d]
            pairs[idx, 1] = data_np[i]
            idx += 1
    # 转换为tensor
    test = torch.from_numpy(pairs).float().to(device)
    # 模型预测
    pre = model(test[:, 0], test[:, 1])
